...
public class BeakCheesyDrive {
    ...
    public DriveCmdBE cheesyDrive(double throttle, double wheel, boolean isQuickTurn, boolean isHighGear) 
    {
		...
		if (isQuickTurn) {
			if (Math.abs(linearPower) < kQuickStopDeadband) {
				...
				mQuickStopAccumlator = (1 - alpha) * mQuickStopAccumlator
						+ alpha * GeneralUtilities.limit(wheel, 1.0) * kQuickStopScalar;
			}
			overPower = 1.0;
			angularPower = wheel;
		} else {
		...
		return new DriveCmdBE(leftPwm, rightPwm);
	}
